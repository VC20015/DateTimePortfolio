...
async function getWeather(city) {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=4b47c728c451a751e7acc8a7b860fe7c&units=metric`
  ).catch(handleError);
  ...
  async function getTime(data) {
    try {
      const response = await fetch(
        `https://secure.geonames.org/timezoneJSON?lat=${data.coord.lat}&lng=${data.coord.lon}&username=afiq9905`
      );
      const responseJson = await response.json();
